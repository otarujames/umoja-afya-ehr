...
from ..schemas import DischargeIn, EncounterStatusUpdate
...
def update_status(encounter_id: str, payload: EncounterStatusUpdate, db: Session = Depends(get_db)):
    ...
    if payload.status in {EncounterStatus.ROOMED, EncounterStatus.IN_PROGRESS} and not encounter.provider_start_at:
