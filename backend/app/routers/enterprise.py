...
CONSENT_TEMPLATES = [
    {"code": "GENERAL_TREATMENT", "name": "General consent for examination and treatment", "category": "Care", "encounter_required": True, "summary": "Consent to clinical assessment, routine tests, medicines and treatment for this visit."},
    {"code": "PRIVACY_DATA_USE", "name": "Privacy, data use and health information notice", "category": "Privacy", "encounter_required": False, "summary": "Acknowledges permitted clinical use, safeguarding, retention and disclosure of health information."},
    {"code": "RELEASE_INFORMATION", "name": "Authorization to release health information", "category": "Privacy", "encounter_required": False, "summary": "Authorizes a defined disclosure to a named person, payer, provider or institution."},
    {"code": "TELEHEALTH", "name": "Telehealth and remote-care consent", "category": "Care", "encounter_required": True, "summary": "Covers remote assessment limitations, privacy, escalation and alternatives."},
    {"code": "SURGERY_PROCEDURE", "name": "Procedure or surgical consent", "category": "Procedure", "encounter_required": True, "summary": "Records the procedure, expected benefits, material risks, alternatives and questions answered."},
    {"code": "ANESTHESIA_SEDATION", "name": "Anesthesia and procedural sedation consent", "category": "Procedure", "encounter_required": True, "summary": "Covers anesthesia/sedation plan, risks, alternatives and post-procedure safeguards."},
    {"code": "BLOOD_TRANSFUSION", "name": "Blood and blood-product transfusion consent", "category": "Procedure", "encounter_required": True, "summary": "Covers indication, transfusion risks, alternatives and refusal consequences."},
    {"code": "HIV_TESTING", "name": "HIV counselling and testing consent", "category": "Testing", "encounter_required": True, "summary": "Records informed agreement, counselling, confidentiality and result communication."},
    {"code": "RESEARCH", "name": "Clinical research participation consent", "category": "Research", "encounter_required": False, "summary": "Study-specific voluntary consent; withdrawal does not affect routine care."},
    {"code": "PHOTOGRAPHY_MEDIA", "name": "Clinical photography, video or media consent", "category": "Media", "encounter_required": False, "summary": "Defines capture purpose, permitted uses, storage and withdrawal limitations."},
    {"code": "MINOR_GUARDIAN", "name": "Parent, guardian or proxy consent", "category": "Legal", "encounter_required": True, "summary": "Records authority and consent for a minor or patient unable to consent."},
    {"code": "EMERGENCY_BASIS", "name": "Emergency treatment legal basis", "category": "Legal", "encounter_required": True, "summary": "Documents urgent treatment when informed consent cannot reasonably be obtained."},
    {"code": "FINANCIAL_RESPONSIBILITY", "name": "Financial responsibility and estimate acknowledgement", "category": "Financial", "encounter_required": True, "summary": "Acknowledges estimates, payer limitations, balances and payment responsibilities."},
    {"code": "PAYMENT_AUTHORIZATION", "name": "Electronic/card/mobile payment authorization", "category": "Financial", "encounter_required": False, "summary": "Authorizes the selected payment channel and records transaction confirmation."},
    {"code": "VACCINATION", "name": "Vaccination consent", "category": "Care", "encounter_required": True, "summary": "Records vaccine information, contraindication review and consent."},
    {"code": "MATERNITY_REPRODUCTIVE", "name": "Maternity and reproductive health consent", "category": "Care", "encounter_required": True, "summary": "Records informed decisions for maternity or reproductive-health care."},
    {"code": "REFUSAL_AMA", "name": "Refusal of treatment / leave against medical advice", "category": "Legal", "encounter_required": True, "summary": "Documents capacity, risks explained, alternatives, refusal and follow-up advice."},
]

PHYSICAL_ENCOUNTER_ACTIVITY_TYPES = {
    "TRIAGE_ASSESSMENT", "PROVIDER_VISIT", "NURSING_ASSESSMENT", "MEDICATION_ADMINISTRATION",
    "PROCEDURE", "THERAPY_SESSION", "IMAGING_ACQUISITION", "SPECIMEN_COLLECTION",
    "VACCINATION", "INFUSION", "BEDSIDE_CARE", "DISCHARGE_EDUCATION",
}


def _payment_channel(method: str, facility_code: str | None, amount: float | None, currency: str) -> dict:
    normalized = method.strip().upper()
    facility = facility_code or get_settings().default_facility
    mobile = any(token in normalized for token in ("M-PESA", "AIRTEL", "TIGO", "HALOPESA", "OPAY", "PALMPAY", "PAGA", "USSD", "EASYPAISA", "JAZZCASH", "MTN MOMO"))
    crypto = any(token in normalized for token in ("BITCOIN", "ETHEREUM", "USDT", "CRYPTO"))
    gateway = any(token in normalized for token in ("GEPG", "ECITIZEN", "PESALINK", "REMITA", "RAAST", "1LINK", "IREMBOPAY"))
    card = "CARD" in normalized
    cash = "CASH" in normalized
    reference_label = "Transaction reference"
    destination = ""
    instructions = "Enter the provider confirmation reference after the transaction is verified."
    if mobile:
        destination = os.getenv("UMOJA_MOBILE_MONEY_TILL", "")
        reference_label = "Mobile-money confirmation code"
        instructions = f"Use the facility Lipa/till number {destination or '[not configured]'}, then enter the confirmed transaction code."
    elif crypto:
        destination = os.getenv("UMOJA_CRYPTO_WALLET", "")
        network = os.getenv("UMOJA_CRYPTO_NETWORK", "")
        reference_label = "Blockchain transaction hash"
        instructions = f"Send only on {network or '[network not configured]'} to {destination or '[wallet not configured]'}. Confirm on-chain before posting."
    elif card:
        destination = os.getenv("UMOJA_CARD_TERMINAL_ID", "")
        reference_label = "Card authorization code"
        instructions = f"Process through terminal {destination or '[not configured]'} and enter the authorization code; never store PAN or CVV."
    elif gateway:
        destination = os.getenv("UMOJA_PAYMENT_GATEWAY_ACCOUNT", "")
        reference_label = "Gateway settlement reference"
        instructions = f"Present or initiate the facility gateway account {destination or '[not configured]'}, verify settlement, then enter the provider reference."
    elif cash:
        destination = os.getenv("UMOJA_CASH_DRAWER_ID", "MAIN-CASH-DRAWER")
        reference_label = "Cash receipt / drawer reference"
        instructions = f"Receive and count cash at {destination}; the system calculates change and issues an audited receipt."
    elif "BANK" in normalized:
        destination = os.getenv("UMOJA_BANK_ACCOUNT", "")
        reference_label = "Bank transfer reference"
        instructions = f"Verify settlement to {destination or '[bank account not configured]'} before posting."
    configured = cash or bool(destination)
    payload = f"umoja://payment?facility={facility}&method={method}&currency={currency}&amount={amount or 0}&destination={destination}"
    qr_data_uri = None
    if configured and (mobile or crypto or gateway):
        try:
            import qrcode
            ...
            buffer = BytesIO()
            image.save(buffer)
            qr_data_uri = "data:image/svg+xml;base64," + b64encode(buffer.getvalue()).decode("ascii")
        except ImportError:
            qr_data_uri = None
    return {"method": method, "facility_code": facility, "configured": configured, "destination": destination or None, "reference_label": reference_label, "instructions": instructions, "qr_data_uri": qr_data_uri, "requires_reference": not cash}
...
class ChargeIn(BaseModel):
    ...
    currency_code: str = Field(default="TZS", pattern=r"^[A-Z]{3}$")
    payer: str | None = None
    posted_by: str
    status: Literal["DRAFT", "POSTED"] = "POSTED"


class ChargeStatusIn(BaseModel):
...
class ConsentIn(BaseModel):
    template_code: str
    encounter_id: str | None = None
    decision: Literal["SIGNED", "DECLINED", "REVOKED"] = "SIGNED"
    signer_name: str = Field(min_length=2, max_length=180)
    signer_relationship: str = Field(default="SELF", max_length=120)
    signature: str = Field(min_length=2, max_length=10000)
    witness_name: str | None = Field(default=None, max_length=180)
...
def module_activities(module_code: str | None = None, status: str | None = None, patient_mpi_id: str | None = None, db: Session = Depends(get_db)):
    ...
    if patient_mpi_id:
        query = query.where(ModuleActivity.patient_id == get_patient(db, patient_mpi_id).id)
...
def create_module_activity(payload: ModuleActivityIn, db: Session = Depends(get_db)):
    ...
    if patient and encounter and encounter.patient_id != patient.id:
        raise HTTPException(status_code=409, detail="Encounter does not belong to the selected patient")
    if payload.activity_type.upper() in PHYSICAL_ENCOUNTER_ACTIVITY_TYPES and not encounter:
        raise HTTPException(status_code=422, detail="A physical patient interaction must be linked to a selected encounter")
    if encounter and encounter.status in {EncounterStatus.DISCHARGED, EncounterStatus.TRANSFERRED, EncounterStatus.LEFT_WITHOUT_BEING_SEEN} and payload.activity_type.upper() in PHYSICAL_ENCOUNTER_ACTIVITY_TYPES:
        raise HTTPException(status_code=409, detail="The selected encounter is closed; create a new visit for new physical care")
    item = ModuleActivity(
...
def consent_templates(category: str | None = None):
    items = CONSENT_TEMPLATES
    if category:
        items = [item for item in items if item["category"].lower() == category.lower()]
    return {"templates": items, "categories": sorted({item["category"] for item in CONSENT_TEMPLATES})}


@router.get("/patients/{patient_mpi_id}/consents")
def patient_consents(patient_mpi_id: str, db: Session = Depends(get_db)):
    patient = get_patient(db, patient_mpi_id)
    items = list(db.scalars(select(ModuleActivity).where(ModuleActivity.patient_id == patient.id, ModuleActivity.module_code == "CONSENT").order_by(ModuleActivity.created_at.desc())).all())
    return [{
        "consent_id": item.activity_id,
        "template_code": item.activity_type,
        "title": item.title,
        "decision": item.status,
        "encounter": encounter_brief(db, item.encounter_id),
        "signed_by": item.created_by,
        "signed_at": item.created_at,
        "details": item.details,
        "evidence": json.loads(item.payload_json or "{}"),
    } for item in items]


@router.post("/patients/{patient_mpi_id}/consents", status_code=201)
def create_patient_consent(patient_mpi_id: str, payload: ConsentIn, db: Session = Depends(get_db)):
    patient = get_patient(db, patient_mpi_id)
    template = next((item for item in CONSENT_TEMPLATES if item["code"] == payload.template_code), None)
    if not template:
        raise HTTPException(status_code=404, detail="Consent template not found")
    encounter = get_encounter(db, payload.encounter_id) if payload.encounter_id else None
    if encounter and encounter.patient_id != patient.id:
        raise HTTPException(status_code=409, detail="Encounter does not belong to the selected patient")
    if template["encounter_required"] and not encounter:
        raise HTTPException(status_code=422, detail="This consent must be linked to a selected patient encounter")
    signed_at = now()
    evidence = f"{patient.mpi_id}|{payload.template_code}|{payload.decision}|{payload.signer_name}|{payload.signature}|{signed_at.isoformat()}"
    signature_hash = hashlib.sha256(evidence.encode("utf-8")).hexdigest()
    item = ModuleActivity(
        module_code="CONSENT",
        ...
        priority="ROUTINE",
        assigned_to="Health Records / Patient Access",
        details=f"{payload.decision} by {payload.signer_name} ({payload.signer_relationship})",
        payload_json=json.dumps({"template_version": "2026.1", "signer_name": payload.signer_name, "signer_relationship": payload.signer_relationship, "witness_name": payload.witness_name, "language": payload.language, "signature_sha256": signature_hash, "signed_at": signed_at.isoformat(), "summary": template["summary"]}),
        created_by=payload.actor,
    )
    db.add(item)
    db.flush()
    write_audit(db, action=f"CONSENT_{payload.decision}", resource_type="PatientConsent", resource_id=item.activity_id, actor=payload.actor, role="consent.capture", patient_mpi_id=patient.mpi_id, facility_code=encounter.facility.code if encounter else None, details=f"{template['name']}; signer={payload.signer_name}; relationship={payload.signer_relationship}; evidence={signature_hash[:16]}")
    db.commit()
    return {"consent_id": item.activity_id, "decision": item.status, "signature_sha256": signature_hash, "signed_at": signed_at}
...
def appointments(
    ...
    if patient_mpi_id:
        query = query.where(Appointment.patient_id == get_patient(db, patient_mpi_id).id)
    if status:
...
def notes(patient_mpi_id: str | None = None, encounter_id: str | None = None, status: str | None = None, db: Session = Depends(get_db)):
    ...
    output = []
    for x in items:
        history = list(db.scalars(select(ManagedEvent).where(ManagedEvent.entity_type == "CLINICAL_NOTE", ManagedEvent.entity_id == x.note_id).order_by(ManagedEvent.occurred_at)).all())
        visible_edits = []
        for event in history:
            if event.action not in {"EDIT_DRAFT", "SIGN", "ADDENDUM"}:
                continue
            metadata = json.loads(event.metadata_json or "{}") if isinstance(event.metadata_json, str) else (event.metadata_json or {})
            visible_edits.append({
                ...
                "changed_fields": sorted((metadata.get("after") or {}).keys()),
            })
        output.append({
            "note_id": x.note_id,
            "patient": patient_brief(db, x.patient_id),
            ...
            "edit_history": visible_edits,
            "amendment_count": sum(1 for event in visible_edits if event["action"] == "ADDENDUM"),
        })
    return output
...
def create_note(payload: NoteIn, db: Session = Depends(get_db)):
    ...
    if encounter.status in {EncounterStatus.DISCHARGED, EncounterStatus.TRANSFERRED, EncounterStatus.LEFT_WITHOUT_BEING_SEEN}:
        raise HTTPException(status_code=409, detail="The selected encounter is closed; use an addendum for a correction or create a new visit for new care")
...
def create_charge(payload: ChargeIn, db: Session = Depends(get_db)):
    ...
    db.add(item)
    db.flush()
    action = "CREATE_ESTIMATE" if payload.status == "DRAFT" else "POST_CHARGE"
    write_audit(db, action=action, resource_type="Charge", resource_id=item.charge_id, actor=payload.posted_by, role="Revenue Cycle", patient_mpi_id=patient.mpi_id, facility_code=encounter.facility.code, details=payload.description)
    db.commit()
    return {"charge_id": item.charge_id, "total": payload.quantity * payload.unit_price, "currency_code": payload.currency_code, "status": item.status}


@router.patch("/charges/{charge_id}")
def update_charge(charge_id: str, payload: ChargeStatusIn, db: Session = Depends(get_db)):
    item = db.scalar(select(Charge).where(Charge.charge_id == charge_id))
    if not item:
        raise HTTPException(status_code=404, detail="Charge or estimate not found")
    if item.status == "VOID":
        raise HTTPException(status_code=409, detail="A voided charge cannot be changed")
    previous = item.status
    item.status = payload.status
    patient = db.get(Patient, item.patient_id)
    encounter = db.get(Encounter, item.encounter_id)
    write_audit(db, action="FINALIZE_ESTIMATE" if payload.status == "POSTED" else "VOID_CHARGE", resource_type="Charge", resource_id=item.charge_id, actor=payload.actor, role="Revenue Cycle", patient_mpi_id=patient.mpi_id, facility_code=encounter.facility.code, details=f"{previous} -> {payload.status}; {payload.reason or ''}")
    db.commit()
    return {"charge_id": item.charge_id, "status": item.status}


@router.get("/claims")
def claims(status: str | None = None, payer: str | None = None, patient_mpi_id: str | None = None, db: Session = Depends(get_db)):
    query = select(Claim)
    if status:
    ...
    if patient_mpi_id:
        query = query.where(Claim.patient_id == get_patient(db, patient_mpi_id).id)
...
def update_claim(claim_id: str, payload: ClaimStatusIn, db: Session = Depends(get_db)):
    ...
    if payload.status == "SUBMITTED":
        ...
        db.add(IntegrationEvent(system=item.payer, event_type="CLAIM_SUBMITTED", resource_type="Claim", resource_id=item.claim_id, payload_json=json.dumps({"amount": money(item.amount), "currency_code": item.currency_code})))
    patient = db.get(Patient, item.patient_id)
    encounter = db.get(Encounter, item.encounter_id)
...
def create_payment(payload: PaymentIn, db: Session = Depends(get_db)):
    ...
    if encounter and encounter.patient_id != patient.id:
        raise HTTPException(status_code=409, detail="Encounter does not belong to the selected patient")
    facility_code = payload.facility_code or (encounter.facility.code if encounter else get_settings().default_facility)
    channel = _payment_channel(payload.method, facility_code, payload.amount, payload.currency_code)
    if not channel["configured"]:
        raise HTTPException(status_code=409, detail=f"{payload.method} is not configured for {facility_code}; configure the facility payment destination before collecting funds")
    reference = (payload.confirmation_code or payload.reference or "").strip() or None
    if channel["requires_reference"] and not reference:
        raise HTTPException(status_code=422, detail=f"{channel['reference_label']} is required")
    if not payload.confirmed:
        raise HTTPException(status_code=422, detail="Confirm that the transaction was received and verified before posting")
    if "CASH" in payload.method.upper() and payload.tendered_amount is not None and payload.tendered_amount < payload.amount:
        raise HTTPException(status_code=422, detail="Cash tendered cannot be less than the payment amount")
    item = Payment(
        patient_id=patient.id,
        encounter_id=encounter.id if encounter else None,
    ...
    db.add(item)
    db.flush()  # payment_id must exist before dependent audit/integration rows.
    event_payload = {"amount": payload.amount, "currency_code": payload.currency_code, "method": payload.method, "facility_code": facility_code, "reference": reference, "channel_metadata": payload.channel_metadata, "tendered_amount": payload.tendered_amount, "change_due": max(0, (payload.tendered_amount or payload.amount) - payload.amount)}
    db.add(IntegrationEvent(system=payload.method, event_type="PAYMENT_CONFIRMED", resource_type="Payment", resource_id=item.payment_id, payload_json=json.dumps(event_payload)))
    write_audit(db, action="RECEIVE_PAYMENT", resource_type="Payment", resource_id=item.payment_id, actor=payload.received_by, role="Cashier", patient_mpi_id=patient.mpi_id, facility_code=facility_code, details=f"{payload.method}; confirmed reference={reference or 'cash receipt'}")
    db.commit()
    return {"payment_id": item.payment_id, "amount": payload.amount, "currency_code": payload.currency_code, "status": "CONFIRMED", "reference": reference, "change_due": event_payload["change_due"]}


@router.get("/payment-instructions")
def payment_instructions(method: str, facility_code: str | None = None, amount: float | None = Query(default=None, ge=0), currency_code: str = Query(default="TZS", pattern=r"^[A-Z]{3}$")):
    return _payment_channel(method, facility_code, amount, currency_code)


@router.get("/payments")
def payments(patient_mpi_id: str | None = None, encounter_id: str | None = None, db: Session = Depends(get_db)):
    query = select(Payment)
    if patient_mpi_id:
        query = query.where(Payment.patient_id == get_patient(db, patient_mpi_id).id)
    if encounter_id:
        query = query.where(Payment.encounter_id == get_encounter(db, encounter_id).id)
    items = list(db.scalars(query.order_by(Payment.received_at.desc())).all())
    return [{
...
def access_catalog(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "countries": [{"code":"TZ","label":"Tanzania","group":"Country Context","description":"United Republic of Tanzania"},{"code":"KE","label":"Kenya","group":"Country Context","description":"Republic of Kenya"},{"code":"NG","label":"Nigeria","group":"Country Context","description":"Federal Republic of Nigeria"},{"code":"PK","label":"Pakistan","group":"Country Context","description":"Islamic Republic of Pakistan"},{"code":"RW","label":"Rwanda","group":"Country Context","description":"Republic of Rwanda"}],
        "templates": {
            code: {
...
def _user_payload(db: Session, item: UserAccount) -> dict:
    photo_version = int(item.profile_photo_updated_at.timestamp()) if item.profile_photo_updated_at else None
    return {
        ...
        "has_profile_photo": bool(item.profile_photo_data),
        "profile_photo_url": f"/profile-photos/{item.user_id}?v={photo_version}" if item.profile_photo_data else None,
        "profile_photo_updated_at": item.profile_photo_updated_at,
        "profile_photo_updated_by": item.profile_photo_updated_by,
        **get_user_access(db, item),
    }


def _profile_photo_type(content: bytes) -> str | None:
...
def user_profile_photo(user_id: str, db: Session = Depends(get_db)):
    item = db.scalar(select(UserAccount).where(UserAccount.user_id == user_id, UserAccount.active.is_(True)))
    if not item or not item.profile_photo_data or not item.profile_photo_mime:
        raise HTTPException(status_code=404, detail="User profile photo not found")
    return Response(
        content=item.profile_photo_data,
        media_type=item.profile_photo_mime,
        headers={
            "Cache-Control": "private, max-age=300",
            "ETag": f'"{item.profile_photo_sha256}"',
            "Content-Disposition": "inline",
        },
    )


@router.post("/admin/users/{user_id}/profile-photo")
async def upload_user_profile_photo(
    user_id: str,
    photo: UploadFile = File(...),
    reason: str = Form(default="Administrator-managed user profile photo"),
    administrator: UserAccount | None = Depends(optional_user),
    db: Session = Depends(get_db),
):
    if not administrator or administrator.role_code.lower() != "admin":
        raise HTTPException(status_code=403, detail="Only an administrator can assign user profile photos")
    item = db.scalar(select(UserAccount).where(UserAccount.user_id == user_id))
    if not item:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    detected_type = _profile_photo_type(content)
    if detected_type is None:
        raise HTTPException(status_code=415, detail="Profile photos must be PNG, JPEG or WebP images")
    actor = administrator.display_name if administrator else "System Administrator"
    digest = hashlib.sha256(content).hexdigest()
    item.profile_photo_data = content
    item.profile_photo_mime = detected_type
    item.profile_photo_sha256 = digest
    item.profile_photo_updated_at = now()
    item.profile_photo_updated_by = actor
    write_audit(
        db,
        action="UPDATE_USER_PROFILE_PHOTO",
        resource_type="UserAccount",
        resource_id=item.user_id,
        actor=actor,
        role="system.users.manage",
        facility_code=item.facility_code,
        details=f"mime={detected_type}; bytes={len(content)}; sha256={digest[:16]}; reason={reason[:300]}",
    )
    db.commit()
    return _user_payload(db, item)


@router.delete("/admin/users/{user_id}/profile-photo")
def delete_user_profile_photo(
    user_id: str,
    reason: str = Query(default="Administrator removed user profile photo", max_length=300),
    administrator: UserAccount | None = Depends(optional_user),
    db: Session = Depends(get_db),
):
    if not administrator or administrator.role_code.lower() != "admin":
        raise HTTPException(status_code=403, detail="Only an administrator can remove user profile photos")
    item = db.scalar(select(UserAccount).where(UserAccount.user_id == user_id))
    if not item:
        raise HTTPException(status_code=404, detail="User not found")
    actor = administrator.display_name if administrator else "System Administrator"
    previous_hash = item.profile_photo_sha256
    item.profile_photo_data = None
    item.profile_photo_mime = None
    item.profile_photo_sha256 = None
    item.profile_photo_updated_at = now()
    item.profile_photo_updated_by = actor
    write_audit(
        db,
        action="REMOVE_USER_PROFILE_PHOTO",
        resource_type="UserAccount",
        resource_id=item.user_id,
        actor=actor,
        role="system.users.manage",
        facility_code=item.facility_code,
        details=f"previous_sha256={(previous_hash or 'none')[:16]}; reason={reason}",
    )
    db.commit()
    return _user_payload(db, item)
