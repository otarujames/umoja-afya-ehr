...
def main() -> None:
    ...
    with SessionLocal() as db:
        ...
        for spec in data["users"]:
            ...
            # Deployment baseline administrators must never inherit a stale custom
            # matrix from an earlier release. platform.admin is global; country
            # administrators are full-function administrators within one country.
            if username == "platform.admin":
                role = "admin"
                countries = sorted(by_country)
                requested = sorted({code for country in countries for code in by_country[country]})
                user.role_code = "admin"
            elif username in {"tz.admin", "ke.admin", "ng.admin"}:
                role = "admin"
            ...
            write_audit(
                ...
                details=f"username={username}; role={role}; countries={','.join(countries)}; facilities={len(requested)}",
            )
        db.commit()
