...
checks={
 ...
 "tokenless_preloaded_provisioning": "firstRunPanel" not in html and "/auth/setup-admin" not in js and "setup-status" not in js and "preloaded_users.json" in (ROOT/"docker-compose.production.yml").read_text(),
 "generated_review_secrets": "POSTGRES_PASSWORD:" not in review and "review_bootstrap_token" not in review and "review_preloaded_users" in review,
 ...
 "clean_release_image": "umoja-afya-ehr:11.0.0" in (ROOT/"docker-compose.production.yml").read_text(),
 ...
 "facility_import_login_fix": "from .models import Facility" in (ROOT/"backend/app/access_control.py").read_text(),
 "global_superuser_roster": '"superuser": True' in (ROOT/"scripts/generate_preloaded_users.py").read_text() and "exactly one manifest-designated global superuser" in (ROOT/"scripts/preload_users.py").read_text(),
 "umojaehr_traefik_deployment": "traefik-59qx-traefik-1" in (ROOT/"scripts/deploy-vps-traefik.sh").read_text() and "umojaehr.online" in (ROOT/".env.production.example").read_text() and "docker-compose.traefik.yml" in (ROOT/"scripts/deploy-vps-traefik.sh").read_text(),
}
errors.extend([name for name,ok in checks.items() if not ok])
report={"release":"11.0.0","checks":checks,"errors":errors}
