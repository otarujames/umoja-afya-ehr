...
class OrderSet(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    set_code: Mapped[str] = mapped_column(String(100), unique=True, index=True)
    name: Mapped[str] = mapped_column(String(255), index=True)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    specialty: Mapped[str | None] = mapped_column(String(120), nullable=True, index=True)
    encounter_types_json: Mapped[str] = mapped_column(Text, default="[]")
    version: Mapped[int] = mapped_column(Integer, default=1)
    source: Mapped[str] = mapped_column(String(80), default="LOCAL_APPROVED")
    active: Mapped[bool] = mapped_column(Boolean, default=True, index=True)
...
class OrderSetItem(Base):
    __tablename__ = "order_set_item"
    __table_args__ = (UniqueConstraint("order_set_id", "orderable_code", name="uq_order_set_orderable"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    order_set_id: Mapped[int] = mapped_column(ForeignKey("order_set.id", ondelete="CASCADE"), index=True)
    orderable_code: Mapped[str] = mapped_column(ForeignKey("order_catalog_item.orderable_code"), index=True)
    sequence: Mapped[int] = mapped_column(Integer, default=0)
    selected_by_default: Mapped[bool] = mapped_column(Boolean, default=True)
    required: Mapped[bool] = mapped_column(Boolean, default=False)
    default_priority: Mapped[str | None] = mapped_column(String(40), nullable=True)
    default_indication: Mapped[str | None] = mapped_column(Text, nullable=True)
    default_instructions: Mapped[str | None] = mapped_column(Text, nullable=True)
