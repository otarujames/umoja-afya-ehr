...
from . import offline_models  # noqa: F401 - registers offline device and replay tables
from .routers import audit, auth, collaboration, enterprise, enhancements, facilities, fhir, flowsheets, health, modules, offline, operations, orders, patients, registration, tracker
from .seed import seed_database
from .security import optional_user
from .rbac import RoleGateMiddleware
from .middleware import BodySizeLimitMiddleware, IdempotencyMiddleware, RateLimitMiddleware, SecurityHeadersMiddleware
...
app.add_middleware(
    ...
    allow_headers=["Authorization", "Content-Type", "X-Request-ID", "X-Break-Glass-Reason", "X-Country-Code", "X-Idempotency-Key", "X-Offline-Created-At", "X-Offline-Device-ID"],
)
...
@app.get("/offline.js", include_in_schema=False)
def offline_javascript() -> FileResponse:
