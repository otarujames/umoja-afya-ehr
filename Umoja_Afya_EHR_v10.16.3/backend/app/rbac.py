...
def required_function(path: str, method: str) -> str | None:
    ...
    if "/consents" in path and path.startswith("/api/v1/patients"):
        return "registration.manage" if write else "patient.chart"
    ...
    if path.startswith("/api/v1/order-catalog") or path.startswith("/api/v1/order-sets"):
        return "system.configuration.manage" if write else "orders.create"
    if path.startswith("/api/v1/orders"):
        if write and "/actions" in path:
    ...
    if path.startswith("/api/v1/charges") or path.startswith("/api/v1/claims") or path.startswith("/api/v1/payments") or path.startswith("/api/v1/payment-instructions"):
        return "revenue.manage"
    if path.startswith("/api/v1/consent-templates"):
