...
document.addEventListener('click',async e=>{
  ...
  function v104StoreKey(name){const scope=`${state.countryCode||'country'}.${state.facility||'facility'}`.replace(/[^a-zA-Z0-9_.-]/g,'_');return `umoja.v10.16.3.${v104UserKey()}.${scope}.${name}`;}
  ...
  function offlineProfile(){
    return {account:state.account,facilities:state.facilities,modules:state.modules,countryCode:state.countryCode,facility:state.facility,deviceId:offlineDeviceId(),release:'10.16.3'};
  }
  ...
  async function handleOfflineAction(action,button){
    ...
    try{
      ...
      if(action==='confirm-enable'){
        ...
        const policy=await api('/offline/policy');const deviceId=offlineDeviceId();await api('/offline/devices',{method:'POST',body:JSON.stringify({device_id:deviceId,device_name:deviceName,platform:navigator.userAgent.slice(0,160),app_version:'10.16.3'})});await offline.enroll(pin,{...offlineProfile(),deviceId},policy);closeModal();toast('Offline access enabled','Install the app if prompted, then open records online once to make them available offline.');updateOfflineUi();return;
      }
