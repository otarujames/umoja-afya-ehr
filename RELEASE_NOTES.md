...
- Fixed the login-critical missing `Facility` import in `backend/app/access_control.py`.
- Removed the legacy setup-secret and browser first-administrator endpoints, UI, settings, Compose secrets and operator instructions.
- Added server-generated, deployment-local credentials for one global superuser and eight operational capacities in each supported country.
- Made `platform.admin` authoritative across all active countries, facilities, functions and departments while keeping country administrators isolated to their country.
- Preserved existing account passwords and existing generated roster passwords during upgrades while repairing stale access matrices.
- Added a VPS deployment command that validates `traefik-59qx-traefik-1`, routes `umojaehr.online` to application port 8000 and verifies public HTTPS health.
- Retained read-only application containers, Docker secrets, password-change enforcement, MFA requirements, rate limiting and audit provenance.
