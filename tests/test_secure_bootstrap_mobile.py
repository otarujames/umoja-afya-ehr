...
def test_login_has_no_prefilled_credentials():
    ...
    assert "firstRunPanel" not in html
    assert "setup-admin" not in js
    assert "setup-status" not in js
    assert "preloaded_users.json" in Path("docker-compose.production.yml").read_text()
...
def test_compose_uses_generated_secret_files():
    ...
    assert "review_bootstrap_token" not in review
    assert "review_preloaded_users" in review
    assert "bootstrap_admin_password" not in prod
    assert "bootstrap_token" not in prod
    assert "preloaded_users" in prod
