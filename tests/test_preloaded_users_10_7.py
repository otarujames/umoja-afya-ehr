...
def test_generator_creates_country_scoped_roster(tmp_path):
    ...
    assert users["platform.admin"]["superuser"] is True
    assert users["platform.admin"]["display_name"] == "Umoja Afya Global Superuser"
    assert {"TZ", "KE", "NG", "PK", "RW"} == set(users["platform.admin"]["country_codes"])
    ...
    assert users["pk.physician"]["country_codes"] == ["PK"]
    assert users["rw.nurse"]["country_codes"] == ["RW"]
    assert len(users) == 41
